...
function toggleMoreInfo(element) {
  // Find the parent teacher card and the expanded-info div
  var teacherCard = element.closest(".teacher-card");
  var expandedInfo = teacherCard.querySelector(".expanded-info");

  // Toggle the "open" class to animate the expansion or collapse
  expandedInfo.classList.toggle("open");

  // Change the text of the button between "+" and "-"
  if (expandedInfo.classList.contains("open")) {
    element.textContent = "-"; // Show "-" when expanded
    expandedInfo.style.display = "block"; // Make the expanded-info visible
  } else {
    element.textContent = "+"; // Show "+" when collapsed
    expandedInfo.style.display = "none"; // Hide the expanded-info
  }
}
